...
3. get 请求的数据会附加在 url 之后，以 “ ？ ” 分割 url 和传输数据，多个参数用 “ & ” 连接，而 post 请求会把请求的数据放在 http 请求体中
4. get 是明文传输，post 是放在请求体中，但是开发者可以通过抓包工具看到，也相当于是明文的
5. get 请求会保存在浏览器历史记录中，还可能保存在 web 服务器的日志中
6. get 和 post 方法没有实质的区别，只是报文格式不同
* get 和 post 是什么？
    * http 协议中的两种发送请求的方法
    * get --- 从指定的资源请求数据
    * post --- 向指定的资源提交要处理的数据
...
* 301 和 302 的区别：
    1. 301：永久移动，请求的网页已永久移动到新的位置，服务器返回此响应，会自动将请求者转到新位置
    2. 302：历史移动，服务器从不同位置的网页响应请求，但请求者应继续使用原有位置来继续以后的请求
    * 字面上的区别就是301是永久重定向，而302是临时重定向
        1. 301 比较常用的场景是使用域名跳转
        2. 302 用来临时跳转，比如未登录的用户访问用户中心，重定向到登录页面
* 200 和 304 ：
    1. 200 表示成功，服务器已成功处理了请求，通常表示为服务器提供了请求的网页
    2. 304 表示未修改：自从上次请求后，请求的网页未修改过，服务器返回此响应时，不会返回网页内容
* 400和401、403：
    * 400状态码：请求无效
        * 产生原因：
